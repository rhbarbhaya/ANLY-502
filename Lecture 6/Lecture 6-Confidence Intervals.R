...
left_edge = sum(df$upper_vector < pop_mean)
right_edge = sum(df$lower_vector > pop_mean)

NoMean = left_edge + right_edge
NoMean

OutOfBounds = round(NoMean/n,2)
OutOfBounds
...
new_lower_vector = samp_mean - 0.84 * samp_sd / sqrt(n)
new_upper_vector = samp_mean + 0.84 * samp_sd / sqrt(n)
plot_ci(new_lower_vector, new_upper_vector, mean(population))

df2 = data.frame(new_lower_vector, new_upper_vector)
new_left_edge = sum(df2$new_upper_vector < pop_mean)
new_right_edge = sum(df2$new_lower_vector > pop_mean)

new_NoMean = new_left_edge + new_right_edge
new_NoMean
OutOfBounds = round(new_NoMean/n,2)
OutOfBounds
